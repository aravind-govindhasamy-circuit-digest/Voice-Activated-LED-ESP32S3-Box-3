 ...
 * @note These can be overridden in menuconfig if using Kconfig
 */
#define DEFAULT_WIFI_SSID CONFIG_EXAMPLE_WIFI_SSID
#define DEFAULT_WIFI_PASS CONFIG_EXAMPLE_WIFI_PASSWORD
#define DEFAULT_MAXIMUM_RETRY 5
...
esp_err_t app_wifi_init(void) {
      ...
      .sta =
          {
               ...
               * support WPA2, these mode can be enabled by setting Threshold
               * below */
              .threshold.rssi = -127,
  ...
  if (bits & WIFI_CONNECTED_BIT) {
    ESP_LOGI(TAG, "connected to ap SSID:%s", DEFAULT_WIFI_SSID);
    return ESP_OK;
  } else if (bits & WIFI_FAIL_BIT) {
    ESP_LOGI(TAG, "Failed to connect to SSID:%s", DEFAULT_WIFI_SSID);
    return ESP_FAIL;
  } else {
