...
static void ui_speech_anim_cb(lv_timer_t *timer) {
  ...
  if (g_sr_anim_active) {
    ...
    for (size_t i = 0; i < 4; i++) {
      lv_bar_set_value(g_sr_bar[i], sr_val[i] > 20 ? sr_val[i] : 20,
                       LV_ANIM_ON);
      lv_bar_set_value(g_sr_bar[7 - i], sr_val[i] > 20 ? sr_val[i] : 20,
                       LV_ANIM_ON);
      lv_bar_set_start_value(g_sr_bar[i], sr_val[i] > 20 ? -sr_val[i] : -20,
                             LV_ANIM_ON);
      lv_bar_set_start_value(g_sr_bar[7 - i], sr_val[i] > 20 ? -sr_val[i] : -20,
                             LV_ANIM_ON);
    }
    g_sr_anim_count++;
  } else {
...
void ui_sr_anim_init(void) {
  ...
  lv_obj_set_style_bg_color(
      g_sr_mask,
      lv_obj_get_style_bg_color(lv_obj_get_parent(g_sr_mask), LV_PART_MAIN),
      LV_STATE_DEFAULT);
  lv_obj_align(g_sr_mask, LV_ALIGN_CENTER, 0, 0);
  lv_obj_add_event_cb(g_sr_mask, sr_mask_event_handler, LV_EVENT_VALUE_CHANGED,
                      NULL);
  ...
  lv_obj_add_event_cb(g_sr_label, sr_label_event_handler,
  ...
  for (size_t i = 0; i < sizeof(g_sr_bar) / sizeof(g_sr_bar[0]); i++) {
    ...
    lv_obj_set_style_bg_color(g_sr_bar[i], lv_color_make(237, 238, 239),
                              LV_STATE_DEFAULT);
    lv_obj_set_style_bg_color(g_sr_bar[i], lv_color_make(246, 175, 171),
                              LV_PART_INDICATOR);
  ...
  for (size_t i = 0; i < sizeof(g_sr_bar) / sizeof(g_sr_bar[0]) / 2; i++) {
    lv_obj_align_to(g_sr_bar[i], obj_img, LV_ALIGN_OUT_LEFT_MID, 15 * i - 65,
                    0);
    lv_obj_align_to(g_sr_bar[i + 4], obj_img, LV_ALIGN_OUT_RIGHT_MID,
                    15 * i + 20, 0);
  }
