...
/* Runtime-converted LVGL images (RGB565), using LVGL's color helpers so colors are correct. */
...
/* Minimal BMP (24bpp, no compression) -> LVGL TRUE_COLOR image for our 320x240 assets. */
static esp_err_t bmp_to_lv_img(const uint8_t *bmp, size_t len, lv_img_dsc_t *out_dsc, uint8_t **out_buf)
{
    ...
    if (needed > len) {
        ESP_LOGE(TAG, "BMP truncated: needed=%lu len=%lu", (unsigned long)needed, (unsigned long)len);
    ...
    if (!buf) {
        ESP_LOGE(TAG, "No mem for RGB565 buffer (%lu bytes)", (unsigned long)out_size);
...
static esp_err_t ensure_images_ready(void)
{
    ...
    esp_err_t err = bmp_to_lv_img(switch_on, sizeof(switch_on), &s_img_on, &s_buf_on);
    if (err != ESP_OK) {
